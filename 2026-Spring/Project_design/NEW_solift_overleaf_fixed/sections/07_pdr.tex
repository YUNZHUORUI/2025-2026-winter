...
\begin{itemize}[noitemsep]
    ...
    \item Phase~3 FEA (Figure~\ref{fig:fea_stress}) confirming a peak wing-root von Mises stress of only \SI{0.325}{MPa} and a \SI{0.207}{mm} tip deflection, i.e.\ a very large structural margin and high stiffness under cruise loads.
    \item Phase~3 CFD confirming a full-vehicle cruise $L/D = 4.8$ (Figure~\ref{fig:cfd_cruise}), consistent with the Chapter~\ref{ch:simulation} ANSYS Fluent result and used as the basis for the range and energy estimates under the selected MH114 airfoil.
...
\label{tab:risk}
\small
\begin{tabularx}{\textwidth}{C{1cm} L{3.5cm} C{1.6cm} C{1.4cm} C{1cm} X}
\toprule
\textbf{ID} & \textbf{Risk Description} & \textbf{\makecell{Likeli-\\hood\\(1--5)}} & \textbf{\makecell{Severity\\(1--5)}} & \textbf{RPN} & \textbf{Mitigation} \\
\midrule
...
\vspace{0.3em}
{\footnotesize RPN = Risk Priority Number = Likelihood $\times$ Severity}
\end{table}
...
\label{tab:actions}
\begin{tabularx}{\textwidth}{C{1cm} X L{3cm} C{2cm} C{1.5cm}}
\toprule
